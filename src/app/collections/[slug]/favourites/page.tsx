...
export default async function CollectionFavouritesPage({ params, searchParams }: PageProps<"/collections/[slug]/favourites">) {
  const { slug } = await params;
  const sp = await searchParams;
  const { collection } = await loadViewableCollection(slug);
  const viewer = await getViewer();
  if (viewer.kind !== "user") notFound();
  return <FavouritesView viewer={viewer} scope={{ collectionId: collection.id }} who={parseWho(sp.who)} base={`/collections/${slug}/favourites`} where="in this collection" />;
}
