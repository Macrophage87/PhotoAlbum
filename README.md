...
- **AI descriptions (opt-in).** With `ANTHROPIC_API_KEY`, `ANNOTATION_ENABLED=true` and an admin's opt-in on the Admin page, each reviewed item is described by Claude (`ANNOTATION_MODEL`, default `claude-opus-5`): a short title (filled in only where the item has none), caption, description, tags, place, a search summary, and for undated scans an estimated year to confirm. Items, trips and collections can be opted out; a scoped backfill with a cost estimate and typed confirmation describes the existing library at half price through the Batches API.
