...
\begin{note}[What has been built so far]
Everything in this plan is now in the album, including the two Google Photos routes below and automatic pet spotting. Decisions 6 and 7 at the end are now about whether we use them, not whether to build them.
\end{note}
...
\textbf{Pets.} The face matching only knows human faces, so the dog and the cats get their own arrangement. Each pet gets a page like a person, with the years they were with us. You tag a pet by hand from the photo viewer, and when your upload note says ``Biscuit asleep on the porch'' the album proposes Biscuit for you to confirm. The album also spots animals in pictures on its own: once Biscuit has been tagged on a photo where it found a dog, later dogs that look like him get a ``Probably Biscuit?'' for you to say yes or no to, and a no is remembered. The chickens are a flock record rather than individuals: whenever chickens are seen, ``the chickens'' is proposed. Animal spotting can be switched off (decision 7).
...
\begin{enumerate}[leftmargin=2em,itemsep=4pt]
  ...
  \item \textbf{Google Photos.} Should we bring our Google Photos libraries over with Takeout, and would each of you connect your Google account for the pick-a-few button? Both are built and ready.
  \item \textbf{Automatic pet spotting.} The album can spot animals and propose pets; it is on by default when the ML helper runs. Keep it, or switch it off and name pets by hand and from notes only?
\end{enumerate}
