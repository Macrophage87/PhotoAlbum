              ...
              {people.map((p) => (
                <Link key={p.id} href={`/people/${p.id}`} className="block rounded-theme border border-border bg-surface p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {p.kind === "HUMAN" && (p.faceIndexing ? <Badge tone="success">recognized · {p.basis === "attestation" ? "attested adult" : "by birthday"}</Badge> : p.optedOut ? <Badge tone="warning">asked to be forgotten</Badge> : p.pendingDecision ? <Badge tone="warning">awaiting admin decision</Badge> : <Badge tone="neutral">not recognized</Badge>)}
