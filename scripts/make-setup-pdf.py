...
S += [P("1. What the application does", H1),
      ...
      bullets([
        ...
        "<b>Themes.</b> Each trip or collection gets a look: lighthouse coast, Scottish highlands, Everglades swamp, desert canyon, alpine, dog park, or classic.",
...
S += [P("How it is built", H2),
      ...
      table([
        ...
        ["AI descriptions", "Anthropic API (optional, two switches)", "A title, caption, tags and a search summary per item"],
...
S += [P("4. Configuration reference (.env)", H1),
      ...
      table([
        ...
        ["GEOCODER_ENABLED / GEOCODER_URL", "true / OpenStreetMap", "Address lookup when setting a photo's place by hand. Only the words typed are sent, from the server."],
...
S += [P("10. Troubleshooting", H1),
      table([
        ...
        ["Photo has no location on the map", "No GPS in the file (on a Samsung, turn on Location tags in the Camera app) and no track covering that time. Import a GPX/FIT/Google trace for that day, or open the photo and use Set a place: click the map or look up the town; a gallery selection can be placed in one go."],
