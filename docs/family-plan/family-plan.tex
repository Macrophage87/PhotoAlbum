...
Everything in this plan is now in the album except two things we agreed to leave for later: the two Google Photos routes below (the pick-a-few button and the Takeout import) are not built yet, and pets are named by hand or from your notes rather than spotted automatically (see the pets paragraph). Both are waiting on decisions 6 and 7 at the end.
\end{note}
...
\textbf{Pets.} The face matching only knows human faces, so the dog and the cats get their own arrangement. Each pet gets a page like a person, with the years they were with us. For now you tag a pet by hand from the photo viewer, and when your upload note says ``Biscuit asleep on the porch'' the album proposes Biscuit for you to confirm. Spotting animals automatically and offering ``Probably Biscuit?'' from the picture itself is possible later if we want it (decision 7); the chickens are a flock record rather than individuals either way.
...
\begin{enumerate}[leftmargin=2em,itemsep=4pt]
  ...
  \item \textbf{Google Photos.} Should we bring our Google Photos libraries over with Takeout, and would each of you connect your Google account for the pick-a-few button? (Not built yet.)
  \item \textbf{Automatic pet spotting.} Is naming pets by hand and from notes enough, or do we want the album to spot animals in pictures too? (Not built yet.)
\end{enumerate}
