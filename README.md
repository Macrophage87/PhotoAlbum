...
- **Dates that survive a stripped file.** The capture time comes from EXIF where there is one; failing that, from the time phones write into the file name (`IMG_20250812_143015`, `PXL_…`, WhatsApp's `IMG-20250812-WA0001`, screenshots), and only then from the file's modified time, which is usually when the file was copied rather than when the photo was taken. Every item says which of those its date came from, and any of them can be corrected by hand.
