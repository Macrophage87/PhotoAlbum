...
- **Schema.** Rename nothing; extend `Photo` into a media table by adding `kind` (PHOTO, VIDEO), `durationS`, `videoRenditions` JSON, `context` text, `estimatedDate`, `estimatedDateConfidence`, `estimatedDateSource`, `annotation` JSON, `annotationModel`, `annotatedAt`, `annotationSource` (MACHINE, EDITED), `reviewedAt`, `searchVector tsvector` (maintained by a trigger), and `embedding vector(512)`. Add `Collection` (slug, title, description, themeKey, visibility, shareToken, coverId, createdById), `CollectionItem` (collectionId, mediaId, position, addedById, unique pair), `Person` (name, relationship, birthday, faceIndexing boolean, createdById; "is a minor" is derived from birthday, not stored), `FaceCluster` (personId nullable, label, ageBandMin, ageBandMax, centroid vector(512)), `Face` (mediaId, clusterId nullable, personId nullable, box, embedding vector(512), confidence, ageAtCaptureYears nullable, status DETECTED/PROPOSED/CONFIRMED/REJECTED, proposedPersonId), `MediaSimilarity` (mediaAId, mediaBId, score, unique pair with A < B), `UploadSession` (for chunked uploads: id, uploaderId, filename, size, mimeType, receivedBytes, storageKey, expiresAt). Keep `PhotoLink` for manual links.
