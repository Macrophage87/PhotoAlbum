...
export async function permittedNames(photoId: string): Promise<string[]> {
  ...
  const faces = await db.face.findMany({ where: { photoId, status: "CONFIRMED", personId: { not: null } }, select: { person: { select: { name: true, birthday: true, adultAttestedAt: true, faceIndexing: true, kind: true } } } });
