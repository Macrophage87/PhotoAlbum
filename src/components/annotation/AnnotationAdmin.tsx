          ...
          <ul className="divide-y divide-border">
            {batches.map((b) => (
              <li key={b.id} className="py-2 flex flex-wrap items-center justify-between gap-2">
                <span>
                  {new Date(b.createdAt).toLocaleString("en-US")} · {b.scope} · {b.requested} requested{b.status !== "SUBMITTED" && <> · {b.succeeded} described, {b.errored} failed, {b.skipped} skipped</>} · <span className="text-muted">{b.status.toLowerCase()}</span>
                </span>
