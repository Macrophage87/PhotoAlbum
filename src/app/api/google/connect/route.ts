...
export async function GET(request: Request) {
  ...
  store.set("google_oauth_state", `${state}:${next.startsWith("/") && !next.startsWith("//") ? next : "/upload"}`, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/api/google", maxAge: 600 });
