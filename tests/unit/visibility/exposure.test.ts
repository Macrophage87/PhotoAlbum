...
const col = (visibility: ContainerRef["visibility"], id = "c1"): ContainerRef => ({ kind: "collection", id, title: "Summer Favourites", visibility });
...
describe("widening", () => {
  it("adding private-trip photos to a public collection exposes them", () => {
    ...
    expect(describeWidening(s, "through the collection Summer Favourites")[0]).toContain("2 of 3 photos from the private trip Lake House 2019 will become visible to anyone on the internet");
  });
...
describe("lowering does not shrink a union", () => {
  it("names the containers that still expose the items", () => {
    ...
    expect(describeStillExposed(s, "this trip")[0]).toBe("1 of this trip's 2 photos is also in the public collection Summer Favourites and stays visible to anyone on the internet.");
  });
